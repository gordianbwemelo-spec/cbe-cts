...
async function viewDashboard(m){
  ...
  m.innerHTML=`<h1 class="page">Dashboard</h1><p class="lead">Live status · <b>${esc(d.campus)}</b> · assessment date <b>${fmtDate(d.ref)}</b> · lead time <b>${d.lead} months</b>.</p>
    ...
    <div class="card"><h2>Development &amp; validation pipeline</h2><div class="sub">Where curricula sit in the development / validation lifecycle</div>${stagePipeline(d.byStage||[])}</div>
    <div class="grid-eq">
...
function execNarrative(d){
  ...
  const notStarted = expired.filter(a=>!reviewUnderway(a.stage));
  const inReview = expired.filter(a=>reviewUnderway(a.stage));
  const parts=[];
  const campusClause = campusScope ? '' : ` in ${b((d.campusList||[]).length)} campuses`;
  parts.push(`As at ${b(ref)}, ${scope} is tracking ${b(s.total)} curricula across ${b(d.programmes)} programme${d.programmes!==1?'s':''} and ${b(d.departments)} department${d.departments!==1?'s':''}${campusClause}.`);

  const validClause = s.Valid>0 ? `${b(s.Valid)} are valid` : 'none are valid';
  const dueClause = s['Due for review']>0 ? `${b(s['Due for review'])} are due for review within the next ${b(d.lead+' months')}` : `none are due for review within the next ${b(d.lead+' months')}`;
  const expClause = s.Expired>0 ? `${b(s.Expired)} have expired` : 'none have expired';
  parts.push(`Of these, ${validClause}, ${dueClause}, and ${expClause}.`);

  const unvClause = s.Unverified>0 ? `${b(s.Unverified)} are unverified or lack valid documents` : 'No curricula are unverified or lacking valid documents';
  const gapsClause = s.gaps>0 ? `${b(s.gaps)} have documentation gaps` : 'no curricula have documentation gaps';
  const recogClause = (s.recognitionGaps>0)
      ? `${b(s.recognitionGaps)} ${campusScope?'are offered here without departmental recognition':'campus offerings lack departmental recognition'}`
      : (campusScope?'no programme is offered here without departmental recognition':'no campus offerings lack departmental recognition');
  ...
  if(!expired.length){
    if(s['Due for review']) parts.push(`${b('Action:')} ${b(s['Due for review'])} curricula are within the review window — reviews should be initiated now.`);
...
async function viewExec(m){
  ...
  const s=d.summary;
  const expired=d.alerts.filter(a=>a.status==='Expired');
  const expiredNotStarted=expired.filter(a=>!reviewUnderway(a.stage));
  const expiredInReview=expired.filter(a=>reviewUnderway(a.stage));
  ...
  if(s.recognitionGaps) priorities.push(`Secure departmental recognition for <b>${s.recognitionGaps}</b> campus offering(s) currently run without it.`);
...
async function viewDetail(m){
  ...
  m.innerHTML=`<h1 class="page">${esc(r.programme)}</h1><p class="lead">${esc(r.department)} · NTA ${esc(r.levels)} · offered at: ${(r.offeredCampuses&&r.offeredCampuses.length)?r.offeredCampuses.map(esc).join(', '):'—'}</p>
    ...
    <div class="grid-eq" style="margin-top:0">
      ...
      <div class="card"><h2>Development &amp; validation stage</h2><div class="sub">Where this curriculum sits in the development / validation lifecycle (national).</div>
        <div style="margin-bottom:10px">${stagePill(r.stage)}</div>
        ${canEdit?`<div class="fld"><label>Development stage</label><select id="rv_stage">${stages.map(s=>`<option ${r.stage===s?'selected':''}>${esc(s)}</option>`).join('')}</select></div>
        <div class="fld"><label>Responsible officer / committee</label><input type="text" id="rv_by" value="${esc(r.reviewer||'')}" placeholder="e.g. Curriculum Validation Committee"></div>
...
window.saveReview=async(id)=>{
  try{ await api('PUT','/curricula/'+id+'/review',{stage:document.getElementById('rv_stage').value,reviewer:document.getElementById('rv_by').value,review_started:document.getElementById('rv_date').value}); toast('Development stage saved'); renderApp(); }
...
async function viewEntry(m){
  let r={programme:'',department:state.meta.lists.departments[0]||'',levels:'',validation:'Available',valid_until:'',observed:'',notes:'',campuses:{}};
  ...
  m.innerHTML=`<h1 class="page">${state.editId?'Update curriculum':'Add a curriculum record'}</h1>
    ...
    <div class="card"><h2>National details</h2><div class="row">
      ...
      <div class="fld"><label>Development / validation stage</label><select id="e_stage">${(state.meta.stages||[]).map(s=>`<option ${r.stage===s?'selected':''}>${esc(s)}</option>`).join('')}</select></div>
...
window.saveCur=async()=>{
  const body={ programme:val('e_programme'),department:val('e_department'),
    validation:val('e_validation'),stage:val('e_stage'),valid_until:val('e_valid'),observed:val('e_observed'),notes:val('e_notes'), campuses:collectCampuses() };
